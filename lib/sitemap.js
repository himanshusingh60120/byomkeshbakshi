...
/**
 * Cached wrapper around collectPropertyUrls. Sitemaps change slowly, and a
 * property like IT Digest is 21 separate XML files, so refetching them on
 * every scan is wasted time.
 */
export async function getPropertyUrls(property, { refresh = false } = {}) {
  const hit = urlCache.get(property.id);
  if (!refresh && hit && Date.now() - hit.at < URL_CACHE_TTL_MS) {
    return { urls: hit.urls, cachedAt: hit.at };
  }
  const urls = await collectPropertyUrls(property, { log: () => {} });
  urls.sort((a, b) => {
  ...
  const at = Date.now();
  urlCache.set(property.id, { at, urls });
  return { urls, cachedAt: at };
}
